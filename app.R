                        ...
                        sidebarPanel(
                          ...
                          introBox(
                            ...
                            conditionalPanel(
                              ...
                              # options to customize parameters
                              div(style="display: inline-block;vertical-align:top; width: 200px;",
                                  numericInput(inputId = "num_of_svs", label = "Number of SVs to Estimate", value = 1,
                                               min = 1, max = 99))
                            ),
                        ...
                        introBox(
                          mainPanel(
                            tabsetPanel(
                              ...
                              tabPanel("Visualization with Marker Genes", downloadButton("download_dim_all", "Download All Genes Plot"),
...
server <- shinyServer(function(input, output, session) {
  ...
  observeEvent(input$do, {
    output$load_data <- renderText({
      ...
      if (input$iasva_param == "Percentage Threshold") {
      ...
      } else if (input$iasva_param == "Number of SVs") {
        dataTables$iasva.res <- withProgress(expr = fast_iasva(summ_exp, mod[,-1, drop = F], verbose=FALSE,
